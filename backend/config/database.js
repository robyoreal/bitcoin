...
async function initializeDatabase() {
  ...
  try {
    ...
    // Create balances table (multi-currency support)
    await connection.execute(`
    ...
    await connection.execute(`
      CREATE TABLE IF NOT EXISTS portfolio (
        ...
        FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
        UNIQUE KEY unique_user_coin_currency (user_id, coin_id, currency)
      )
    `);
...
/**
 * Convert MySQL DECIMAL fields to numbers
 */
function parseNumericFields(rows) {
  if (!rows) return rows;
  ...
  const parseRow = (row) => {
    ...
    const numericFields = ['balance', 'amount', 'average_buy_price', 'price', 'total'];
...
function all(sql, params, callback) {
  ...
  const promise = pool.execute(sql, params)
    .then(([rows]) => parseNumericFields(rows))
    .catch(error => {
...
function get(sql, params, callback) {
  ...
  const promise = pool.execute(sql, params)
    .then(([rows]) => parseNumericFields(rows[0] || null))
    .catch(error => {
